...
int main(void) {
    ...
    if (event_used_index < 0 || request_available_index < 0 || request_used_index < 0 ||
        !send_event(&event_queue,
                    file_descriptors[(size_t)event_used_index + 1u],
    ...
    for (;;) {
        ...
        if (!eventfd_wait(file_descriptors[(size_t)request_available_index + 1u])) {
            goto finish;
        }
        for (;;) {
            kb2_test_vq_segment_t segments[2];
            ...
            if (opcode == KB2_TEST_REQUEST_BAD_USED_ID) {
                if (kb2_test_vq_inject_used_id(&request_queue,
                    ...
                    (notify &&
                     !eventfd_write_one(
                         file_descriptors[(size_t)request_used_index + 1u]))) {
                    goto finish;
                }
                continue;
            }
            ...
            if (opcode == KB2_TEST_REQUEST_QUIESCE) {
                if (notify_used &&
                    !eventfd_write_one(file_descriptors[(size_t)request_used_index + 1u])) {
                    goto finish;
                }
                if (!send_event(&event_queue,
                                file_descriptors[(size_t)event_used_index + 1u],
