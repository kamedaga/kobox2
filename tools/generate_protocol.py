...
SCHEMA_PATH = ROOT / "protocol" / "schema" / "transport.json"
OUTPUT_PATH = ROOT / "protocol" / "generated" / "include" / "kobox2" / "protocol_layout.h"
...
def render(schema):
    ...
    for name, value in schema["constants"].items():
        ...
        lines.append(f"#define KB2_PROTOCOL_{identifier(name)} {value}{suffix}")
    lines.append("")

    for structure_name, structure in schema["structures"].items():
        prefix = f"KB2_PROTOCOL_{identifier(structure_name)}"
        lines.append(f"#define {prefix}_SIZE {structure['size']}u")
        for field in structure["fields"]:
            lines.append(f"#define {prefix}_{identifier(field['name'])}_OFFSET {field['offset']}u")
        lines.append("")
